...
/// (GAP-081 phase 2b). Case-sensitive, searched in the file head.
const STUB_GENERATED_MARKERS: [&str; 5] = [
...
enum StubAnchorForm {
    ...
    /// and `.newFutureStub(`.
    JavaGrpcFactory,
    /// C#/TS/JS: `new CartServiceClient(channel)`.
    NewServiceClient,
}
...
fn stub_anchor_forms(ext: &str) -> &'static [StubAnchorForm] {
    match ext {
        ...
        "cs" | "ts" | "tsx" | "js" | "jsx" => &[StubAnchorForm::NewServiceClient],
        _ => &[],
    }
}
...
///   file.
/// - **Non-Go** (phase 2b): the generated-stub caller anchor of `py`, `java`,
///   `kt`, `cs`, `ts`, `tsx`, `js`, `jsx` — see [`StubAnchorForm`] for the
///   forms and [`stub_anchor_forms`] for which language spells which form.
///
