...
/// GAP-081: scan non-generated `.go` files for gRPC client/server anchor forms.
///
/// `New<Stem>ServiceClient(` marks a caller of `<Stem>Service`,
/// `Register<Stem>ServiceServer(` marks a provider of it. Definitions are never
/// call sites (see the skip below), and `protoc-gen-go` output is excluded
/// entirely — it declares both anchors for EVERY service in the project, so
/// counting it would attach every caller to every stub file.
...
/// `New<Stem>ServiceClient(` anchor). Only a service with at least one caller
